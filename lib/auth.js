...
// 加密密钥（每次启动随机生成）
let SECRET_KEY = null;
let SESSION_TOKEN = null;        // 客户端 token
let SERVER_TOKEN = null;         // 服务端 token
...
function init() {
  ...
  // 生成 16 字节 session token（客户端用）
  SESSION_TOKEN = crypto.randomBytes(16).toString('hex');
  // 生成 16 字节 server token（服务端用，独立于客户端 token）
  SERVER_TOKEN = crypto.randomBytes(16).toString('hex');
  return SESSION_TOKEN;
}

/**
 * 获取服务端 token（用于本地浏览器访问）
 */
...
/**
 * 验证服务端 token
 * @param {string} token 服务端 token
 * @returns {boolean} 是否有效
 */
function validateServerToken(token) {
  return token && token === SERVER_TOKEN;
}
